...
namespace Mid_Term
{
    class Shopping2
    {
        ...
        public static void GetItemFromUser(List<Product> shoppingMenu)
        {
            ...
            //Call The Receipt Method using the same input variables as the checkout method. This is the only Piece of the Shopping Method that still needs to be built.
            
        }

        private static void CheckoutCartForUser(Dictionary<Product, double> userCheckoutList, double grandTotal)
        {
        ...
        private static bool ValidateQuantityInput(string userQuantityInput)
        {
        ...
        public static bool ValidateFoodInput(List<Product> shoppingMenu, char userFoodSelection, string userAmountSelection)
        {
            if (int.TryParse(userFoodSelection.ToString(), out int intFoodSelection))
            {
                if (intFoodSelection <= shoppingMenu.Count)
                {
                    return true;
                }
                else
